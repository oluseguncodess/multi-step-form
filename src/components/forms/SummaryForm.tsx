...
export default function SummaryForm() {
  ...
  return (
    ...
    <div className='bg-white flex justify-between items-center px-4 pt-2 max-w-[450px]'>
      <span className='text-gray-400 font-[300] text-[0.9rem]'>Total (per {toggleSubscription === 'monthly' ? 'month' : 'year'})</span>
      <span className='text-purplee-600 font-semibold text-[1.05rem] md:text-[1.2rem]'>+${totalPrice}/{toggleSubscription === 'monthly' ? 'mo' : 'yr'}</span>
    </div>
    </>
  );
}
